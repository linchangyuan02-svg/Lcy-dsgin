...
async function resolveFont(font: string, fontUrl?: string) {
  ...
  try {
    ...
    if (typeof document !== 'undefined' && (document as any).fonts && (document as any).fonts.load) {
      try {
        await (document as any).fonts.load(resolved);
      } catch {
        // Ignore fallback
      }
    }
...
class AppGallery {
  ...
  constructor(
    ...
    this.originalItemsCount = items ? items.length : 12;
  ...
  onTouchUp(e: any) {
    ...
    if (this.start !== undefined && this.startY !== undefined && endX !== undefined && endY !== undefined) {
      const dx = endX - this.start;
  ...
  handleClick(clientX: number, clientY: number) {
    if (!this.container || !this.renderer || !this.renderer.gl) return;
    ...
    for (const media of this.medias) {
      ...
      if (clickX >= px - halfW && clickX <= px + halfW) {
        clickedMedia = media;
        break;
      }
    }
    ...
    if (clickedMedia && this.onItemClick) {
      this.onItemClick(clickedMedia.index % this.originalItemsCount);
    }
  }
...
export default function CircularGallery({
  ...
  }, [items, bend, textColor, borderRadius, font, fontUrl, scrollSpeed, scrollEase, onItemClick, showText]);
